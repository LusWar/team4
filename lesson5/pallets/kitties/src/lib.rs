...
use frame_support::{ decl_module, decl_storage, decl_error, StorageValue, StorageMap, ensure, traits::Randomness };
...
decl_storage! {
	trait Store for Module<T: Trait> as Kitties {
		...
		pub OwnerKitties get(fn owner_kitties): map hasher(blake2_128_concat) (T::AccountId, u32) => u32;

		pub OwnerKittiesCount get(fn owner_kitties_count): map hasher(blake2_128_concat) T::AccountId => u32;
	}
}
...
impl<T: Trait> Module<T> {
	...
	fn next_kitty_id() -> sp_std::result::Result<u32,DispatchError> {
		...
		ensure!(kitty_id <= u32::max_value(),Error::<T>::KittiesCountOverflow);
	...
	fn insert_kitty(owner: T::AccountId, kitty_id: u32, kitty: Kitty) {
		...
		<OwnerKittiesCount<T>>::insert(&owner,owner_id +1);
